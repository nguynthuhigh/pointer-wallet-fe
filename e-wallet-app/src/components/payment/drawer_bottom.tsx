...
const DrawerBottom:React.FC<BottomDrawerProps> = ({onClose,state,transactionID,voucher_code}) => {
        ...
        <div>
            <Drawer
                ...
                >
                   ...
                   <Box
                        sx={{ width: 'auto', padding: '16px', textAlign: 'center', height: '400px' }}
                        role="presentation">
                        <h1 class={`font-semibold my-2`}>Nhập mã bảo mật</h1>
                        <h1 class={`font-semibold text-red-500`}>{error}</h1>
                        <div class={`mx-auto ${error && `border-red-500`} relative w-fit  border  rounded-full`}>
                                <div class={`p-4`}>
                                    <OTPInput
                                        value={otp}

...
const ApplyVoucher:React.FC<BottomDrawerProps> = ({onClose,state,id,handleDataVoucher,partnerID,amount}) => {
    ...
    <div>
        <div>
            <Drawer
                ...
                onClose={onClose}
                >
                <button class={`bg-gray-100 m-2 rounded-full p-2 ml-auto`} onClick={onClose}>
                ...
                <Box
                    ...
                    <form class={`flex`} onSubmit={handleApplyVoucher}>
                        <input ref={codeRef}  placeholder={`Nhập mã khuyến mãi`} class={` border rounded-lg w-full text-sm p-1.5`}></input>
                        <button class={`w-[100px] p-1.5 text-sm bg-blue-default text-white font-semibold rounded-lg ml-2`}>Áp dụng</button>
                    </form>
                    <div class={`mt-5`}>
                        {isLoading ? 'Loading' : voucherData?.map((item,key)=>(
                            <VoucherItem onClick={handleAvailableApplyVoucher} data={item} amount={amount} key={key}></VoucherItem>
                        ))}

...
const FormatTypeCurrency = ({...props}) => {
  ...
  if(data.type === 'payment'){
    return <h1 className={`w-full font-semibold text-base text-end text-red-500`}>-{amount}</h1>
  }
  if(data.type === 'transfer' && data.sender._id === props.userID){
    return <h1 className={`w-full font-semibold text-base text-end text-red-500`}>-{amount}</h1>
  }
  if(data.type === 'transfer' && data.sender._id !== props.userID){
    return <h1 className={`w-full font-semibold text-base text-end text-green-500`}>+{amount}</h1>
  }

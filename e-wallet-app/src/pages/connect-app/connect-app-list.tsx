...
import {
  disconnectWallet,
  getConnectedAppWallet,
} from "../../services/api/connect-app.api";
...
export const ConnectAppList: React.FC = () => {
  ...
  const [modalText, setModalText] = useState(
    "Bạn có chắc là hủy liên kết không ?"
  );
      ...
      <div class={`container-center`}>
        ...
        <div className="space-y-5">
          {data?.map((item: any) => (
            <div
              key={item._id}
              className="grid grid-cols-[90px_1fr_150px] items-center text-black"
            >
              <img
                src={item.partnerID.image || AvatarDefault}
                className="size-[60px] rounded-full"
              />
              <div>
                <p className="font-bold">
                  {item.partnerID.name || "Cập nhật tên đối tác!"}
                </p>
                <p className="text-gray-500">
                  {item.partnerID.description || "Không có mô tả"}
                </p>
              </div>
              <div>
                <button

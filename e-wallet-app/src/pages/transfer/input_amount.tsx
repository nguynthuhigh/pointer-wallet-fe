...
const InputAmount = ({...props}) => {
    ...
    const handleSubmit = ()=>{
        ...
        if(amount < 100){
            ...
            console.log(123)
            return setError(`Số tiền chuyển tối đa là ${formatCurrency(100000000,'VND')}`)
      ...
      <HeaderTransfer onClick={()=>{props.handleStepTransfer('search_user')}} title="Nhập số tiền"></HeaderTransfer>
        <div class={`mt-14`}>
            ...
            <CurrencyInput
                ...
                defaultValue={0}
                decimalsLimit={2}
                class={`border-0 mt-5 focus:outline-none text-center text-6xl w-full font-semibold bg-white ${error && ' text-red-500'}`}
                onValueChange={(values) => {setDataSend({...dataSend,amount:Number(values)});setError(null)}}
                />
        ...
        <div class={`rounded-xl bg-gray-100 mt-2 p-4 flex`}>
            <img class={`w-10 h-10 rounded-full object-cover`} src={props.userData.avatar ? props.userData.avatar :  avatar}></img>
            <div class={`ml-4`}>
                <h1 class={`text-sm text-gray-500`}>Người nhận:</h1>
                <h1 class={`text-md`}>{props.userData.full_name}</h1>
            </div>
        </div>
        <button onClick={handleSubmit} class={`w-full mt-10  p-3 bg-blue-600 font-semibold text-white rounded-full `}>Gửi</button>
        <DrawerBottom data={dataSend} onClose={handleSubmit} state={isOpen} ></DrawerBottom>
    </div>
  )
}
